...
class ProductoServices {
  //Se crea una clase para trasladar la logica
  ...
  async BuscarUno(id) {
    ...
    const productos = this.productos.find((item) => item.id === id); //Si hay un elemento igual a nuestro id
    if (!productos) {
      throw boom.notFound('Producto no encontrado');
    }
    if (productos.isBlock) {
  ...
  async Actualizar(id, cambios) {
    ...
    this.productos[index] = {
      //De esta manera le indico que persistan los valores que estan en productos y solo me modifique lo que esta en cambios
      ...productos,
      ...cambios,
    };
